...
extension LogInViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var sectionableViewCell = UITableViewCell()
        
        if (indexPath.row % 2 == 0) {
            sectionableViewCell = tableView.dequeueReusableCell(withIdentifier: cellIDLoginTextFild, for: indexPath)
        } else {
            sectionableViewCell = tableView.dequeueReusableCell(withIdentifier: cellIDPasswordTextFild, for: indexPath)
        }
        return sectionableViewCell
    }
